...
def go_to_face(block, next_face):
    if next_face in block.get_neighbors():
        ...
        return to_csv_row(NUMBER_TO_GO_TO_FACE_ACTION[next_face], block)
    else:
...
def rotate_right(block):
    ...
    block.r, block.c = FACE_TO_COORDINATE[block.orientation][block.face]
    if block.puzzle: block.puzzle.add_to_history(to_csv_row(BlockAction.RotateRight, block))
...
def rotate_left(block):
    ...
    block.r, block.c = FACE_TO_COORDINATE[block.orientation][block.face]
    if block.puzzle: block.puzzle.add_to_history(to_csv_row(BlockAction.RotateLeft, block))
